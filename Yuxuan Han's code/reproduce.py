...
def l2_run(data_dir: Path, out_dir: Path, c: float) -> dict:
    ...
    # figures
    # ------------------------------------------- L2 four-panel summary
    r_c, _ = fit_curve(data_xy, lib[0]["xy"])   # self-contained: recompute cone rms
    fig, ((a1, a2), (a3, a4)) = plt.subplots(2, 2, figsize=(11.5, 8.6))
    ...

    # (a) model comparison in the (R_H, R_s) plane
    a1.plot(lib[0]["xy"][:, 0], lib[0]["xy"][:, 1], "--", lw=1.5,
            color="tab:blue", label=f"cone (r={r_c:.4f})")
    a1.plot(best_gap_entry["xy"][:, 0], best_gap_entry["xy"][:, 1], ":",
            lw=1.5, color="tab:orange",
            label=f"hard gap {best_gap_entry['Eg_kT']:.2f} kBT "
                  f"(r={best_gap_r:.4f})")
    a1.plot(rhz, rsz, "o", ms=5, color="tab:red", label="mean data")
    a1.set(xlabel="normalised R_H", ylabel="normalised R_s",
           title=f"(a) Data vs both models -> {sel['model']}")
    a1.grid(alpha=0.3); a1.legend(fontsize=8)

    # (b) model selection: the 15% rule
    a2.bar([0, 1], [r_c, best_gap_r], width=0.5,
           color=["tab:blue", "tab:orange"], alpha=0.8)
    a2.set_xticks([0, 1]); a2.set_xticklabels(["cone", "hard gap"])
    rel = abs(r_c - best_gap_r) / min(r_c, best_gap_r)
    a2.axhline(min(r_c, best_gap_r) * 1.15, color="red", ls="--", lw=1,
               label="15% threshold")
    a2.text(0.5, max(r_c, best_gap_r) * 0.97,
            f"relative gap = {rel * 100:.1f}%\n"
            f"-> {'decisive' if rel >= 0.15 else 'UNCERTAIN'}\n"
            f"selected: {sel['model']}",
            ha="center", va="top", fontsize=9,
            bbox=dict(boxstyle="round", fc="lightyellow", alpha=0.8))
    a2.set(ylabel="fit rms residual",
           title="(b) Model selection (15% rule)")
    a2.grid(alpha=0.3, axis="y"); a2.legend(fontsize=8)

    # (c) assigned E_F trajectory vs E_F = 0 midpoint
    lo, hi = np.percentile(EF_boot, [16, 84], axis=0)
    a3.fill_between(order, lo, hi, alpha=0.25, color="tab:green",
                    label="bootstrap 16-84%")
    a3.plot(order, EF_traj, "o-", ms=3, lw=1, color="tab:green",
            label="assigned E_F")
    a3.axhline(0, color="k", ls="--", lw=0.8,
               label="$E_F=0$ (midpoint)")
    if bracket:
        a3.axvspan(bracket[0], bracket[1], color="tab:red", alpha=0.15,
                   label="sign-change bracket")
    i0 = int(np.argmin(np.abs(EF_traj)))
    a3.plot([order[i0]], [EF_traj[i0]], "*", ms=13, color="tab:red",
            label=f"closest to 0 @ index {order[i0]}")
    a3.set(xlabel="measurement_index", ylabel="assigned $E_F$ [eV]",
           title="(c) $E_F$ trajectory vs midpoint")
    a3.grid(alpha=0.3); a3.legend(fontsize=8)

         # (d) measured longitudinal resistance R_xx vs measurement index.
    #     Self-contained: reads R_xx_raw from L2_data.csv (long format,
    #     one row per index x repeat) and pivots to (n_index, n_repeat).
    RXX = None
    _v = locals().get("RXX")
    if (_v is not None and np.asarray(_v).ndim == 2
            and np.asarray(_v).shape[0] == len(order)):
        RXX = np.asarray(_v, dtype=float)      # loader already provides it
    if RXX is None:
        try:
            import csv as _csv
            from pathlib import Path as _Path
            _root = _Path("data") if _Path("data").is_dir() else _Path(".")
            _cands = sorted(_root.rglob("L2_data.csv"))
            if not _cands:
                raise FileNotFoundError(
                    f"L2_data.csv not found under {_root}; CSVs seen: "
                    f"{[str(p) for p in _root.rglob('*.csv')][:5]}")
            _acc = {}
            with open(_cands[0], newline="") as fh:
                for _row in _csv.DictReader(fh):
                    _acc.setdefault(int(_row["measurement_index"]), {}) \
                        [int(_row["repeat"])] = float(_row["R_xx_raw"])
            _ni = max(_acc) + 1
            _nr = max(r for d in _acc.values() for r in d)
            RXX = np.full((_ni, _nr), np.nan)
            for _i, _d in _acc.items():
                for _r, _v2 in _d.items():
                    RXX[_i, _r - 1] = _v2
            print(f"[load] panel (d): R_xx_raw from {_cands[0]} -> {RXX.shape}")
        except Exception as exc:
            print(f"[WARN] panel (d): cannot build R_xx ({exc})")

    if RXX is not None and RXX.shape[0] == len(order) and RXX.size > 0:
        xx_m = np.nanmean(RXX, axis=1)
        xx_s = (np.nanstd(RXX, axis=1, ddof=1) if RXX.shape[1] > 1
                else np.zeros_like(xx_m))
        for _r in range(RXX.shape[1]):
            a4.plot(order, RXX[:, _r], "-", lw=0.6, color="tab:gray",
                    alpha=0.45, label="raw repeats" if _r == 0 else None)
        a4.errorbar(order, xx_m, yerr=xx_s, fmt="o-", ms=3.5, lw=1.2,
                    color="tab:red", label="mean ± sd")
        if bracket:
            a4.axvspan(bracket[0], bracket[1], color="tab:red", alpha=0.12,
                       label="R_H sign-change bracket")
            _i_pk = int(np.nanargmax(xx_m))
            _tag = "inside" if bracket[0] <= _i_pk <= bracket[1] else "OUTSIDE"
            a4.plot(order[_i_pk], xx_m[_i_pk], "*", ms=13, color="tab:purple",
                    label=f"peak @ index {order[_i_pk]} ({_tag} bracket)")
        a4.set(xlabel="measurement_index", ylabel="R_xx",
               title="(d) Longitudinal $R_{xx}$ vs neutral point")
        a4.grid(alpha=0.3); a4.legend(fontsize=8)
    else:
        a4.text(0.5, 0.5, "R_xx still unavailable:\n"
                "see [load]/[WARN] panel (d) lines in the terminal",
                ha="center", va="center", transform=a4.transAxes, fontsize=9)
        a4.set(title="(d) Longitudinal $R_{xx}$")



    fig.suptitle("L2 inverse problem - G12", fontsize=12)
    fig.tight_layout(rect=(0, 0, 1, 0.96))
    fig.savefig(out_dir / "L2_figure.png", dpi=200)
    plt.close(fig)
